"""Authenticated, bounded Fabric bridge for one selected Sphero Ollie."""
...
from .policy import (
    ...
    OLLIE_DEADMAN_SECONDS,
    OLLIE_NUDGE_METERS_PER_SECOND,
)
from .transport import OllieTransport


@dataclass(frozen=True, slots=True)
class FabricOllieConfiguration:
...
class OllieCommandHandler:
    def __init__(self, *, node_id: str, transport: OllieTransport) -> None:
    ...
    async def execute(self, command: FabricResolvedCommand) -> tuple[dict[str, object], bool]:
        if command.targetNodeId != self.node_id:
            raise ValueError("Command target is not this Sphero Ollie")
        if command.expiresAt <= datetime.now(UTC):
            raise ValueError("Command expired before Sphero Ollie execution")
    ...
    async def _execute_once(self, action: str, parameters: dict[str, object]) -> dict[str, object]:
        ...
        if action == GROUND_VELOCITY_CAPABILITY:
            ...
            roll = await self.transport.set_velocity(
                self._number(parameters, "forwardMetersPerSecond"),
                self._number(parameters, "rightMetersPerSecond"),
                self._number(parameters, "clockwiseRadiansPerSecond"),
            )
            self._record_velocity(roll.speed_value)
            return self._roll_details(roll.heading_degrees, roll.speed_value)
        if action == GROUND_NUDGE_CAPABILITY:
            ...
            forward, right = {
                "forward": (OLLIE_NUDGE_METERS_PER_SECOND, 0.0),
                "backward": (-OLLIE_NUDGE_METERS_PER_SECOND, 0.0),
                "left": (0.0, -OLLIE_NUDGE_METERS_PER_SECOND),
                "right": (0.0, OLLIE_NUDGE_METERS_PER_SECOND),
            }[direction]
            roll = await self.transport.set_velocity(forward, right, 0.0)
            self._record_velocity(roll.speed_value)
            return {
                "direction": direction,
                **self._roll_details(roll.heading_degrees, roll.speed_value),
            }
        ...
        raise ValueError(f"Unsupported Sphero Ollie Fabric action {action!r}")
    ...
    async def deadman_tick(self, *, now: float | None = None) -> bool:
        ...
        if (
            ...
            and current - self._last_velocity_at >= OLLIE_DEADMAN_SECONDS
        ):
            async with self._write_lock:
                await self.transport.stop()
            self._clear_velocity()
    ...
    async def _demo_drive(self, direction: str, _pulse: int) -> None:
        forward = (
            ...
            else -OLLIE_NUDGE_METERS_PER_SECOND
        )
        async with self._write_lock:
    ...
    def _record_velocity(self, speed_value: int) -> None:
        self._velocity_active = speed_value > 0
    ...
    @staticmethod
    def _roll_details(heading: int, speed: int) -> dict[str, object]:
        return {
...
class FabricOllieBridge:
    def __init__(self, configuration: FabricOllieConfiguration, transport: OllieTransport) -> None:
        self.configuration = configuration
        self.transport = transport
        self.handler = OllieCommandHandler(node_id=configuration.node_id, transport=transport)
    ...
    async def run(self) -> None:
        ...
        try:
            ...
            async with client.connected():
                tasks = (
                    asyncio.create_task(self._receive(client), name="ollie-receive"),
                    asyncio.create_task(self._tick(client), name="ollie-deadman-sensors"),
                    asyncio.create_task(self._heartbeat(client), name="ollie-heartbeat"),
                )
        ...
        while True:
            ...
            if frame_type == "adapter.stop":
                if frame.get("nodeId") != self.configuration.node_id:
                    raise RuntimeError("Fabric stop frame targeted a different Sphero Ollie")
                await self.handler.safe_stop()
                continue
    ...
    async def _handle_command(
        ...
        try:
            ...
            await client.publish_lifecycle(
                command,
                "REJECTED",
                code="SPHERO_OLLIE_COMMAND_REJECTED",
            ...
            try:
                ...
                await client.publish_lifecycle(
                    command,
                    "FAILED",
                    code="SPHERO_OLLIE_OPERATION_FAILED",
                    message=self._last_error,
                )
