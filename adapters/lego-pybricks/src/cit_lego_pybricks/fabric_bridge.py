...
class FabricLegoBridge:
    ...
    async def run(self) -> None:
        ...
        try:
            ...
            await self._adapter.stop(reason="fabric_bridge_shutdown", at=datetime.now(UTC))
        ...
        while True:
            ...
            if frame_type == "adapter.stop":
                ...
                await self._adapter.stop(
                    reason=str(frame.get("reason", "fabric_stop")), at=datetime.now(UTC)
                )
                continue
    ...
    async def _handle_command(
        ...
        try:
            self._handler.translate(command)
