"""Read-only BLE discovery for exact Sphero BOLT selection.
...
stable one-way identifier and the exact advertised ``SB-XXXX`` name.
"""
...
from .models import SpheroAdvertisement, SpheroBoltCandidate

_BOLT_NAME_PATTERN = re.compile(r"^SB-[0-9A-Z]{4}$", re.IGNORECASE)
Discoverer = Callable[[float], Awaitable[Iterable[SpheroAdvertisement]]]


def candidate_id(address: str) -> str:
    digest = hashlib.sha256(f"sphero-bolt\0{address.casefold()}".encode()).hexdigest()[:12]
    return f"sphero-{digest}"


def is_bolt_name(name: str) -> bool:
    return _BOLT_NAME_PATTERN.fullmatch(name.strip()) is not None
...
) -> list[SpheroBoltCandidate]:
    found: dict[str, SpheroBoltCandidate] = {}
    for advertisement in advertisements:
        display_name = advertisement.name.strip()
        if not is_bolt_name(display_name):
            continue
        opaque_id = candidate_id(advertisement.address)
        candidate = SpheroBoltCandidate(
...
async def _bleak_discover(timeout_seconds: float) -> Iterable[SpheroAdvertisement]:
    try:
        ...
        raise RuntimeError(
            "Sphero BOLT Bluetooth support is not installed; run the CIT business installer"
        ) from error
...
async def scan_sphero_bolts(
    timeout_seconds: float = 4.0,
    *,
    discoverer: Discoverer | None = None,
) -> list[SpheroBoltCandidate]:
    if not 0.5 <= timeout_seconds <= 20:
        raise ValueError("Sphero BOLT scan duration must be between 0.5 and 20 seconds")
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="cit-sphero-bolt-discover")
...
async def _run(arguments: argparse.Namespace) -> None:
    robots = await scan_sphero_bolts(arguments.duration)
    payload = []
    for robot in robots:
