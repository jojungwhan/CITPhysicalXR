"""CLI entry point for one independently supervised Sphero BOLT node."""
...
from .fabric_bridge import FabricSpheroBridge, FabricSpheroConfiguration
from .transport import FakeSpheroTransport, Spherov2BleakTransport


def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="cit-sphero-bolt")
...
async def _run(arguments: argparse.Namespace) -> None:
    ...
    transport = (
        ...
        else Spherov2BleakTransport(arguments.candidate_id)
    )
    ...
    await FabricSpheroBridge(
        FabricSpheroConfiguration(
